...
def plot_throughput_boxplots(folder_path, threshold=10):
    ...
    file4_path = os.path.join(folder_path, 'hdfs_rs_3_2_c.csv')
    file5_path = os.path.join(folder_path, 'hdfs_rs_6_3_c.csv')
    file6_path = os.path.join(folder_path, 'daos_1_0_c.csv')
    file7_path = os.path.join(folder_path, 'gluster_fs_6_4_c.csv')
    file8_path = os.path.join(folder_path, 'hdfs_3_replication_c.csv')
    ...
    box = plt.boxplot(boxplot_data, labels=['D-Rex SC', 'GreedyMinStorage', 'D-Rex LB', 'EC(3,2)', 'EC(6,3)', 'DAOS', 'EC(4,2)', 'HDFS 3 Rep'], showmeans = False, patch_artist = True)
    ...
    plt.ylabel('Throughput Difference from Threshold')
    plt.legend()
