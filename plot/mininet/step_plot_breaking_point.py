...
# Loop over each file and plot the data as a step plot
for file in files:
    ...
    # Calculate the rolling maximum of Size_stored
    df_downsampled['Rolling_max'] = df_downsampled[' Size_stored'].cummax()

    # Calculate the percentage of data still stored based on the rolling maximum
    df_downsampled['Percentage_stored'] = (df_downsampled[' Size_stored'] / df_downsampled['Rolling_max']) * 100
    
    # Plot the downsampled percentage data as a step plot
    plt.step(df_downsampled['Time_in_days'], df_downsampled['Percentage_stored'], label=f'{alg_name}', where='post')
...
plt.ylabel('Percentage of Data Stored (%)')
plt.title('Percentage of Data Stored Over Time with Node Failures (Step Plot)')
plt.legend(loc='best')
...
# Display the plot
plt.tight_layout()
plt.savefig(folder_path + '/breaking_point_step_' + input_nodes_to_print + "_" + input_data_to_print + "_" + str(data_duration_on_system) + "_" + str(reliability_threshold) + ".pdf")
