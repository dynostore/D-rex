...
def algorithm2_reduced_complexity(number_of_nodes, reliability_of_nodes, bandwidths, reliability_threshold, file_size, real_records, node_sizes, reduced_set_of_nodes, reduced_set_of_nodes_first_nodes_only):
	"""
	Choose fastest N and biggest K.
	...
	min_K = 0
	set_of_nodes_chosen = []
	set_of_nodes = reduced_set_of_nodes_first_nodes_only
		...
		for set_of_nodes_chosen in itertools.combinations(set_of_nodes, i):
			...
			if (K != -1):
				...
				if (replication_and_write_time < min_time):
					if (nodes_can_fit_new_data(set_of_nodes_chosen, node_sizes, file_size/K) == False): # If this set of node doesn't work we want to check the next node in the pack of nodes of reduced complexity
						print("Check next nodes")
					else:
						min_time = replication_and_write_time
						min_N = i
						min_K = K
