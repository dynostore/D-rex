...
def algorithm2_reduced_complexity(number_of_nodes, reliability_of_nodes, bandwidths, reliability_threshold, file_size, real_records, node_sizes, reduced_set_of_nodes, iteration, maximum_difference_allowed):
	...
	if (iteration == 0):
		...
		matrix_of_differences = group_nodes_by_similarities(number_of_nodes, reliability_of_nodes, bandwidths, node_sizes, maximum_difference_allowed)
		reduced_set_of_nodes = get_reduced_set_of_nodes(number_of_nodes, matrix_of_differences, maximum_difference_allowed)
	...
	min_K = -1
	set_of_nodes_chosen = []
	set_of_nodes = [sub_array[0] for sub_array in reduced_set_of_nodes] # The set of nodes only get the first of each subset of similar nodes
	print("set_of_nodes:", set_of_nodes)
	set_of_nodes_corresponding_index_in_reduced_set_of_nodes = list(range(0, number_of_nodes))
	index = 0
				...
				if (replication_and_write_time < min_time):
					...
					for node in set_of_nodes_chosen:
						if (node_sizes[node] - file_size/K < 0):
							node_ok = False
							print("Would not fit on node", node, "memory only", node_sizes[node], "index", set_of_nodes_corresponding_index_in_reduced_set_of_nodes[node], "in reduced set of nodes")
							print("Check next reduced_set_of_nodes:", reduced_set_of_nodes, "avail len is", len(reduced_set_of_nodes[set_of_nodes_corresponding_index_in_reduced_set_of_nodes[node]]))
							for l in range (1, len(reduced_set_of_nodes[set_of_nodes_corresponding_index_in_reduced_set_of_nodes[node]])):
								next_possible_node = reduced_set_of_nodes[set_of_nodes_corresponding_index_in_reduced_set_of_nodes[node]][l]
								print("Checking next possible node:", next_possible_node)
								if (node_sizes[next_possible_node] - file_size/K >= 0): # If it would fit, use it
	...
	print("\nAlgorithm 2 with reduced complexity chose N =", min_N, "and K =", min_K, "with the set of nodes:", min_set_of_nodes_chosen, "It took", end - start, "seconds.")
