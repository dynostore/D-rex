...
def algorithm3(number_of_nodes, reliability_of_nodes, bandwidths, reliability_threshold, file_size, real_records):
    ...
    for i in range(2, number_of_nodes + 1):
        ...
        for set_of_nodes_chosen in itertools.combinations(set_of_nodes, i):
            ...
            if (K != -1):
                ...
                print(i, K, replication_and_write_time)
            set_of_possible_solutions.append(
                (i, K, set_of_nodes_chosen, replication_and_write_time, (file_size/K)*i))
            time_and_space_from_set_of_possible_solution.append(
                [replication_and_write_time, (file_size/K)*i])
    ...
    for i in range(0, len(set_of_solution_on_pareto)):
        time_on_pareto.append(
            time_and_space_from_set_of_possible_solution[set_of_solution_on_pareto[i]][0])
    ...
    for i in range(0, size+1):
        progress = 100 - \
            ((time_on_pareto[i] - time_on_pareto[0])*100)/total_progress
    ...
    print("\nAlgorithm 3 chose N =", min_N, "and K =", min_K, "with the set of nodes:",
